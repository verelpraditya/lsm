...
class C_penduduk extends CI_Controller
{
	...
	public function ExportToExcel()
	{
		...
		$this->excel_generator->set_header(array(
			...
			'Kewarganegaraan', 'Kompetensi', 'Status Penduduk', 'Status Tinggal', 'Difabilitas',
		...
		*/
		...
		$this->excel_generator->exportTo2003('Data Anggota');
	}
	...
	function lists()
	{
		...
		$colModel['nama'] = array('Nama Anggota', 200, TRUE, 'left', 2);
		...
		$colModel['ref_desa.nama_desa'] = array('Desa', 80, TRUE, 'center', 2);
		$colModel['ref_kecamatan.nama_kecamatan'] = array('Kecamatan', 80, TRUE, 'center', 2);
		$colModel['ref_kab_kota.nama_kab_kota'] = array('Kab/Kota', 80, TRUE, 'center', 2);
		$colModel['ref_provinsi.nama_provinsi'] = array('Provinsi', 80, TRUE, 'center', 2);
	...
	function load_data()
	{
		...
		$valid_fields = array(
			'id_penduduk', 'nik', 'nama', 'tempat_lahir', 'tanggal_lahir', 'ref_jen_kel.deskripsi',
			'no_telp',
			'ref_desa.nama_desa', 'ref_kecamatan.nama_kecamatan', 'ref_kab_kota.nama_kab_kota', 'ref_provinsi.nama_provinsi'
		); //Ingat Tambahkan pendapatan per bulan
...
<button type="submit" class="btn btn-info btn-xs" title="Detail Penduduk" onclick="detil_penduduk(\'' . $row->id_penduduk . '\')"/><i class="fa fa-eye"></i></button>'
	...
	function detil($id)
	{
		...
		if ($this->session->userdata('logged_in') and $role == 'Pengelola Data') {
			...
			$data['page_title'] = 'Detil Data Anggota';
			$data['menu'] = $this->load->view('menu/v_pengelolaData', $data, TRUE);
