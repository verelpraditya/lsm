...
class C_keluarga extends CI_Controller
{
	...
	public function ExportToExcel()
	{
		...
		$this->excel_generator->getActiveSheet()->setCellValue('A1', 'Data Anggota');
		...
		$this->excel_generator->exportTo2007('Data Anggota');
	}
	...
	function lists()
	{
		...
		$colModel['tbl_penduduk.nama'] = array('Nama Anggota', 200, TRUE, 'left', 1);
	...
	function load_data()
	{
		...
		foreach ($records['records']->result() as $row) {
			$record_items[] = array(
				...
				//				<input type="button" value="Tambah Anggota" class="tambah" onclick="tambah_anggota(\''.$row->id_keluarga.'\')"/>'

				// 				<button type="submit" class="btn btn-success btn-xs" title="Tambah Anggota" onclick="tambah_anggota(\''.$row->id_keluarga.'\')"/><i class="fa fa-plus-square"></i></button>
				// 				<button type="submit" class="btn btn-info btn-xs" title="Tampil Anggota" onclick="tampil_anggota_keluarga(\''.$row->id_keluarga.'\')"/><i class="fa fa-eye"></i></button>
...
<button data-toggle="modal" href="#dialog-print" type="submit" class="btn btn-primary btn-xs" title="Cetak Kartu Anggota" onclick="cetak(\'' . $row->nik . '\')"/><i class="fa fa-print"></i></button>
'
			);
		}
		//Print please
	...
	function add()
	{
		...
		if ($this->session->userdata('logged_in') and $role == 'Pengelola Data') {
			...
			//Atribut Anggota
			$data['nama_provinsi'] = $this->m_keluarga->get_provinsi();
	...
	// 	$data['nama_kab_kota'] = $this->m_keluarga->get_kabkota_dinamic($id_provinsi);
	// 	$this->load->view('keluarga/kabkota', $data);
	// }

	// function getKec()
	// {
	// 	$id_kab_kota = $this->input->post('id_kab_kota');
	// 	$data['nama_kecamatan'] = $this->m_keluarga->get_kecamatan_dinamic($id_kab_kota);
	// 	$this->load->view('keluarga/kecamatan', $data);
	// }

	// function getDesa()
	// {
	// 	$id_kecamatan = $this->input->post('id_kecamatan');
	// 	$data['nama_desa'] = $this->m_keluarga->get_desa_dinamic($id_kecamatan);
	// 	$this->load->view('keluarga/desa', $data);
	// }

	// public function get_provinsi()
	// {
	// 	$data['provinsi'] = $this->select_model->get_provinsi();
	...
	{
		$id_provinsi = $this->input->post('id_provinsi');
		$kabupaten = $this->m_keluarga->get_kabupaten($id_provinsi);
	...
	{
		$id_kab_kota = $this->input->post('id_kab_kota');
		$kecamatan = $this->m_keluarga->get_kecamatan($id_kab_kota);
	...
	{
		$id_kecamatan = $this->input->post('id_kecamatan');
		$desa = $this->m_keluarga->get_desa($id_kecamatan);
